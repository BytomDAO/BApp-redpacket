...
func (s *Server) ListReceiverRedPackets(c *gin.Context, req *ListRedPacketsReq) (*ListReceiverRedPacketsResp, error) {
	...
	receiversQuery := s.db.Master().Joins("join senders on receivers.id = senders.id")
	receiversQuery = receiversQuery.Preload("Sender").Where(&orm.Receiver{Address: req.Address}).Where(&orm.Sender{AssetID: req.AssetID})
	if err := receiversQuery.Find(&receivers).Error; err != nil {
